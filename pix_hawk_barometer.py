...
class Barometer(object):
    ...
    def __init__(self, con_str, address=I2C_ADD_BMP388):
        self._address = address
        self.con_str = con_str
        self.baro_thread = Thread(target = self.target)
    ...
    def connect(self):
        try:
                #self.ctrl.configure('ftdi://ftdi:232h:FT4VTTQV/1')
            self.ctrl.configure(self.con_str)
                #ok=self.ctrl.configured
            self.slave = self.ctrl.get_port(I2C_ADD_BMP388)
            self.connected = True
        except:
            self.connected = False
            return False
    ...
    def target(self):
        print('barometer thread started')
        while self.run_thread:
            ...
            time.sleep(.5)
        print('barometer thread stopped')
    ...
    def _read_byte(self, cmd):
        #rdb  =self.slave.read(1)
        rdb  =self.slave.read_from(cmd,1)
        pa = bytearray(rdb)
        mn = int.from_bytes(pa,byteorder='big')
        return mn
    ...
    def _read_u16(self, cmd):
        ...
        LSB = self._read_byte(cmd)
        #MSB = self._bus.read_byte_data(self._address, cmd + 0x01)
        MSB = self._read_byte(cmd + 0x01)
        return (MSB << 0x08) + LSB
